"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.InventoryOptimizationService = void 0;
const common_1 = require("@nestjs/common");
const demand_prediction_service_1 = require("./demand-prediction.service");
let InventoryOptimizationService = class InventoryOptimizationService {
    demandPredictor;
    constructor(demandPredictor) {
        this.demandPredictor = demandPredictor;
    }
    async recommendParLevels(kitchenId) {
        const demand = await this.demandPredictor.forecastDemand(kitchenId);
        return {
            recommendations: [
                { ingredient: 'Burger Buns', currentStock: 50, recommendedPar: 150, action: 'ORDER' },
                { ingredient: 'Taco Shells', currentStock: 200, recommendedPar: 180, action: 'OK' },
                { ingredient: 'Tomatoes (kg)', currentStock: 5, recommendedPar: 20, action: 'ORDER' }
            ],
            reasoning: 'Based on the predicted 120 burger orders at 12:00 PM, burger bun stock is critically low.'
        };
    }
};
exports.InventoryOptimizationService = InventoryOptimizationService;
exports.InventoryOptimizationService = InventoryOptimizationService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [demand_prediction_service_1.DemandPredictionService])
], InventoryOptimizationService);
//# sourceMappingURL=inventory-optimization.service.js.map